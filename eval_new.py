...
if __name__ == "__main__": 
    ...
    for shots in shot_sg:
        ...
        #             i=0
        #             for i in range(50):
        #                 outsum = np.append(outsum,out_numpy[:,i,:])
        #             i=0
        #             for i in range(50):
        #                 noisysum = np.append(noisysum,noisy_numpy[:,i,:])
        #             i = 0
        #             for i in range(50):
        #                 cleansum = np.append(cleansum, clean_numpy[:, i, :])
        #
        # df = pd.DataFrame({'Column A': outsum, 'Column B': noisysum})
        # # 指定要保存的 Excel 文件路径和文件名
        # excel_file_path = './output/data1.xlsx'
        # # 将 DataFrame 写入 Excel 文件
        # df.to_excel(excel_file_path, index=False)
        # df = pd.DataFrame({'Column A': cleansum})
        # # 指定要保存的 Excel 文件路径和文件名
        # excel_file_path = './output/data2.xlsx'
        # df.to_excel(excel_file_path, index=False)
