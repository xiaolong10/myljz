...
def Data_Preparation(noise_version=1):
    ...
    rnd_train = np.random.randint(low=20, high=200, size=len(beats_train)) / 100   #0.2~1.99 随机缩放噪声
    ...
    # Adding noise to test          修改此处确定测试集加噪范围
    noise_index = 0
    ...
    X_train = np.array(sn_train)#加噪训练集
    y_train = np.array(beats_train)
    
    X_test = np.array(sn_test)#加噪测试集
    y_test = np.array(beats_test)
    
    X_train = np.expand_dims(X_train, axis=2)
    y_train = np.expand_dims(y_train, axis=2)

    X_test = np.expand_dims(X_test, axis=2)   #2
    y_test = np.expand_dims(y_test, axis=2)  #2
